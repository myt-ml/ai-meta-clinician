...
export function redactContactInfo(
  text: string,
  preserveContext: boolean = true
): string {
  let redacted = text;
...
export function redactLocations(
  text: string,
  preserveContext: boolean = true
): string {
  let redacted = text;
...
export function redactMessage(
  ...
  const countRedactions = (
    before: string,
    after: string,
    type: RedactedMessage["redactedEntities"][0]["type"]
  ) => {
    const pattern = config.preserveContext
      ? `[${type.toUpperCase()}]`
      : "[REDACTED]";
    const beforeCount = (
      before.match(new RegExp(pattern.replace(/[[\]]/g, "\\$&"), "g")) || []
    ).length;
    const afterCount = (
      after.match(new RegExp(pattern.replace(/[[\]]/g, "\\$&"), "g")) || []
    ).length;
...
export class PilotLogger {
  ...
  getAnalytics(): {
    ...
    const totalResponseTime = this.logs.reduce(
    ...
    this.logs.forEach((log) => {
      ...
      if (log.riskLevel) {
        riskLevelDistribution[log.riskLevel] =
          (riskLevelDistribution[log.riskLevel] || 0) + 1;
      }
      ...
      languageDistribution[log.language] =
        (languageDistribution[log.language] || 0) + 1;
