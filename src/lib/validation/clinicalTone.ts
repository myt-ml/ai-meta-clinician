...
const CLINICAL_DISCLAIMERS = {
  crisis: "This is a mental health emergency. Please contact emergency services immediately (dial 911 or local emergency number).",
  highRisk: "Based on your responses, it's important to seek immediate professional evaluation. Please contact a mental health professional or visit your nearest emergency department.",
  general: "This information is based on clinical guidelines and does not replace professional medical assessment. Please consult with a qualified healthcare provider.",
};
 ...
 */
export function enforceClinicAlTone(text: string, context: {
  ...
  if (/[\u{1F600}-\u{1F64F}]|[\u{1F300}-\u{1F5FF}]|[\u{1F680}-\u{1F6FF}]|[\u{2600}-\u{26FF}]/u.test(correctedText)) {
    violations.push('Emoji detected');
    correctedText = correctedText.replace(/[\u{1F600}-\u{1F64F}]|[\u{1F300}-\u{1F5FF}]|[\u{1F680}-\u{1F6FF}]|[\u{2600}-\u{26FF}]/gu, '');
  }
...
function hasClinicAlPrefix(text: string): boolean {
  ...
  return clinicalIndicators.some(indicator => firstSentence.includes(indicator));
}

/**
 * Add clinical prefix
 */
function addClinicalPrefix(text: string, context: { riskLevel?: string }): string {
