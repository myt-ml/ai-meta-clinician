...
function generateOfflineResponse(riskLevel: string): OfflineResponse {
  if (riskLevel === "critical") {
    return {
      message:
        "EMERGENCY: If you are in immediate danger, please call 911 or go to your nearest emergency room.",
      mhGAPRecommendation:
        "Immediate psychiatric evaluation required. High risk of suicide.",
      safetyResources: [
  ...
  if (riskLevel === "high") {
    return {
      message:
        "You may be experiencing significant distress. Please contact a mental health professional as soon as possible.",
      mhGAPRecommendation:
        "Clinical assessment recommended within 24-48 hours. Safety planning indicated.",
      safetyResources: [
        "988 Suicide & Crisis Lifeline",
  ...
  if (riskLevel === "moderate") {
    return {
      message:
        "It sounds like you're going through a difficult time. Consider reaching out to a mental health professional.",
      mhGAPRecommendation:
        "Clinical evaluation recommended. Monitor for symptom escalation.",
      safetyResources: [
  ...
  return {
    message:
      "Thank you for sharing. If symptoms persist or worsen, consider speaking with a healthcare provider.",
    mhGAPRecommendation:
      "Supportive counseling may be beneficial. Psychoeducation on stress management.",
    safetyResources: [
...
describe("ASR Failure Simulation Tests", () => {
  ...
  describe("Text-Only Mode (ASR Unavailable)", () => {
    ...
    it(
      "should detect crisis from text input without ASR",
      () => {
  ...
  describe("Offline-Only Mode", () => {
    ...
    it("should provide crisis resources in offline crisis mode", () => {
      ...
      expect(result.assessment.riskLevel).toBe("critical");
      const response = result.response as OfflineResponse;
      expect(response.safetyResources).toContain(
        "988 Suicide & Crisis Lifeline"
      );
      expect(response.safetyResources).toContain("911 Emergency Services");
    });
  ...
  describe("Crisis Detection Without ASR", () => {
    ...
    it(
      "should provide immediate offline response for crisis (no ASR)",
      () => {
        const startTime = Date.now();
    ...
    it("should calculate correct Columbia scores without ASR", () => {
      ...
      testCases.forEach(({ text, expectedScore }) => {
        const result = handleTextInput(text, config);
        expect(result.assessment.columbiaScore).toBeGreaterThanOrEqual(
          expectedScore - 1
        );
      });
    });
  ...
  describe("Performance Without ASR", () => {
    ...
    it("should maintain fast response times without ASR", () => {
      const queries = ["I feel stressed", "I feel depressed", "I need help"];
  ...
  describe("User Notifications", () => {
    ...
    it("should inform user about offline-only mode capabilities", () => {
      const message =
        "Running in offline mode. Responses are based on WHO mhGAP guidelines.";
  ...
  describe("Graceful Degradation", () => {
    ...
    it("should function entirely offline when needed", () => {
      ...
      const queries = ["I feel stressed", "I feel depressed", "I want to die"];
