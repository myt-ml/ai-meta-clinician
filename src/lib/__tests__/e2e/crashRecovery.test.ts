...
class HealthMonitor {
  ...
  getSystemHealth(): SystemHealth {
    ...
    const allFailed = models.every(m => m.status === 'unhealthy' || m.status === 'failed');
...
describe('Crash and Recovery Tests', () => {
  ...
  describe('Mid-Inference Crash Recovery', () => {
    it('should handle inference crash with retry', async () => {
      ...
      try {
        ...
        // Determine recovery
        const strategy = determineRecoveryStrategy('ollama', error as Error, healthMonitor);
    ...
    it('should fallback to next provider after repeated crashes', async () => {
      const message = 'Test message';
    ...
    it('should complete inference after crash and recovery', async () => {
      const message = 'Test message';
      ...
      try {
        ...
        if (strategy.action === 'retry') {
          ...
          result = await simulateInference(strategy.nextProvider, message, false);
    ...
    it('should handle crashes during crisis scenarios', async () => {
      ...
      try {
        ...
        const offlineResponse = '[Offline] EMERGENCY: Call 988 or 911 immediately';
  ...
  describe('Model Loading Failures', () => {
    it('should detect model loading failure', () => {
      ...
      const strategy = determineRecoveryStrategy('ollama', error, healthMonitor);
    ...
    it('should use offline protocol when all models fail to load', () => {
      ...
      const strategy = determineRecoveryStrategy('webllm', error, healthMonitor);
  ...
  describe('Graceful Degradation', () => {
    ...
    it('should degrade features progressively', () => {
      const degradationLevels = [
        { failed: [], available: ['ollama', 'webllm', 'offline'], level: 'healthy' },
        { failed: ['ollama'], available: ['webllm', 'offline'], level: 'degraded' },
        { failed: ['ollama', 'webllm'], available: ['offline'], level: 'critical' }
      ];
  ...
  describe('Recovery Metrics', () => {
    it('should track recovery latency', async () => {
    ...
    it('should provide recovery recommendations', () => {
      ...
      if (systemHealth.overall === 'critical') {
        const recommendation = 'Restart all model providers and check system resources';
        expect(recommendation).toContain('Restart');
      } else if (systemHealth.overall === 'degraded') {
        const recommendation = 'Check failing providers and monitor system health';
